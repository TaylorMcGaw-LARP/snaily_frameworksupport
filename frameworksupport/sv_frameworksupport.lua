...
CreateThread(function()
	local pluginConfig = Config.GetPluginConfig("frameworksupport")
		...
		if pluginConfig.enabled then
			...
			function GetIdentity(target, cb)
				...
				if xPlayer ~= nil then
					...
					if cb ~= nil then
						...
						TriggerClientEvent('snailyCAD::frameworksupport:returnIdentity', target, xPlayer)
					end
				else
					...
					if cb ~= nil then
						...
						TriggerClientEvent('snailyCAD::frameworksupport:returnIdentity', target, {})
					end
				end
			end
			...
			RegisterNetEvent('snailyCAD::frameworksupport:getIdentity')
			AddEventHandler('snailyCAD::frameworksupport:getIdentity', function()
				GetIdentity(source)
			end)

			-- Event for clients to trigger job refresh on server (primarily for QBUS onduty handling)
			RegisterNetEvent('snailyCAD::frameworksupport:refreshJobCache')
			AddEventHandler('snailyCAD::frameworksupport:refreshJobCache', function()
				local src = source
				GetCurrentJob(src, true)
			end)

			-- EVENT_RECORD_ADDED
			RegisterServerEvent('snailyCAD::pushevents:RecordAdded')
			AddEventHandler('snailyCAD::pushevents:RecordAdded', function(record)
